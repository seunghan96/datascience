...
하지만 Monte Carlo Iteration에도 다음과 같은 2가지 문제점이 존재한다. </br></br>
(1) **Value Function**
...
하지만, 환경/모델에 대한 정보가 없는 상황(앞의 식에서의 <a href="https://www.codecogs.com/eqnedit.php?latex=P_{ss'}" target="_blank"><img src="https://latex.codecogs.com/gif.latex?P_{ss'}" title="P_{ss'}" /></a>)를 모르는 상황이다. </br>
